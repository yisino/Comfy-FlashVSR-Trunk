...
插件自带一个**实时状态面板**（前端扩展 `js/flashvsr_trunk_panel.js`，由 `WEB_DIRECTORY` 自动加载），
渲染过程中在右上角浮出，无任务时自动隐藏。面板内容：

- **分块数量**：已完成 / 总数 / 失败数；
- **渲染进度**：百分比 + 进度条 + 当前帧 / 总帧数；
- **已渲染时长**：`HH:mm:ss` 实时计时，运行态下估算并显示**预计剩余(ETA)**；
- **视频对比区**：超分前（源视频）与超分后（输出视频）左右并列，播放 / 暂停 / 拖动进度**双向同步**；
- **取消按钮**：点击即调用 ComfyUI 中断（等价于队列取消），后端检测到中断会标记「已取消」并干净退出；
- **状态徽标**：渲染中 / 已完成 / 失败 / 已取消 四态配色。

> 视频对比区通过插件内注册的安全路由 `/flashvsr_trunk_video` 提供（仅允许本次运行登记过的源/输出视频，
> 防止任意文件泄露）。该路由在节点执行时按需注册，并自动支持 HTTP Range，前端可正常拖动进度。

**输出文件名时间戳**：所有写盘产物（`FlashVSR_Trunk` / `FlashVSR_Trunk_Advanced` / `FlashVSR_Trunk_Merge`）
在原输出名后追加精确到秒的时间戳，例如 `FlashVSR_Trunk_20260830_172345.mp4`；
若同秒已存在同名文件则自动追加 `_1` / `_2` … 后缀，**永不覆盖**已有文件。
时间戳追加发生在路径规范化（`safe_output_name`）之后，因此用户自定义输出名中的非法字符仍会被剔除。
